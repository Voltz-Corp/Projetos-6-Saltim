...
import { useContagem, getGlobalProgress, buildAllUpdates } from '../hooks/useContagem'
...
function getStep(unit: string): number {
  return ['KG', 'G', 'GR', 'L', 'LT', 'ML'].includes(unit.toUpperCase()) ? 0.1 : 1
}
...
function GlobalProgressBar({ touched, total }: { touched: number; total: number }) {
  const pct = total === 0 ? 0 : Math.round((touched / total) * 100)
  const done = pct === 100
  return (
    <div className="bg-white border border-stone-200 rounded-xl p-4">
      <div className="flex items-center justify-between mb-2.5">
        <span className={cn('text-xs font-medium', done ? 'text-green-700' : 'text-stone-500')}>
          {done ? 'Estoque completo! ✓' : 'Progresso total do estoque'}
        </span>
        <span className="text-xs tabular-nums text-stone-500">
          <span className="font-bold text-stone-900">{touched}</span>
          {' / '}{total}
          <span className={cn('ml-1', done ? 'text-green-700 font-bold' : 'text-stone-400')}>· {pct}%</span>
        </span>
      </div>
      <div className="h-1.5 bg-stone-100 rounded-full overflow-hidden">
        <div
          className="h-full rounded-full transition-all duration-300"
          style={{ width: `${pct}%`, backgroundColor: done ? '#2D7A3A' : '#F07820' }}
        />
      </div>
    </div>
  )
...
function ProductImage({ name, category }: { name: string; category: Category }) {
...
export function ContagemCategoriaPage() {
  ...
  const { adjust, setCount, confirmar, desmarcar, getCount, isTouched, touchedCount, totalCount, buildUpdates } = useContagem(items)
  ...
  const isGlobalDone = globalProgress.touchedCount === globalProgress.totalCount && globalProgress.totalCount > 0
  ...
  async function handleNext() {
    ...
    if (isGlobalDone || isLastCategory) {
      ...
      navigate({ to: '/contagem/$index', params: { index: String(categoryIndex + 1) } })
    }
  }
    ...
    <div className="flex flex-col h-screen bg-surface">
      ...
      <div className="bg-white border-b border-stone-200 px-8 py-4 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-stone-400 font-medium uppercase tracking-wide">{category}</p>
          <h1 className="text-lg font-semibold text-stone-900 leading-tight">Contagem de insumos</h1>
        </div>
        <div className="flex items-center gap-4 flex-shrink-0">
          <span className="text-xs text-stone-500 tabular-nums">{touchedCount} / {totalCount}</span>
          <button
      ...
      <div className="px-8 py-4 flex flex-col gap-3 bg-surface flex-shrink-0">
        <GlobalProgressBar touched={globalProgress.touchedCount} total={globalProgress.totalCount} />
...
interface ItemCardProps {
  item: { id: number; name: string; unit: string; minQty: number; currentQty: number; category: Category }
  value: number
...
function ItemCard({ item, value, onAdjust, onSet, done }: ItemCardProps) {
  ...
  return (
    <div className={cn(
      'rounded-xl border p-4 flex flex-col gap-3 transition-colors',
        ...
        ? isCritical ? 'bg-red-50 border-red-200' : 'bg-brand-50 border-brand-100'
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-stone-900 leading-snug line-clamp-2">{item.name}</div>
          {done && (
            <span className={cn(
              'inline-flex items-center gap-1 mt-1 text-xs font-semibold',
              isCritical ? 'text-red-600' : 'text-brand-600',
            )}>
      ...
      <div className="text-xs space-y-0.5">
        <div className="flex items-baseline gap-1">
          ...
          <span className="font-bold text-stone-800">{fmtQty(item.currentQty, item.unit)}</span>
        </div>
        <div className="flex items-baseline gap-1">
          <span className="text-stone-400">Mín</span>
          <span className="text-stone-500">{fmtQty(item.minQty, item.unit)}</span>
        </div>
        {deficit > 0 && (
      ...
      <div className="flex items-center justify-end gap-1.5 mt-auto">
        <div className="flex items-center bg-stone-100 rounded-lg border border-stone-200 overflow-hidden">
          <button
            ...
            className="w-7 h-8 flex items-center justify-center text-stone-500 hover:text-brand-600 text-base font-medium transition-colors"
          >
            −
          </button>
        ...
        <span className="text-xs font-bold text-stone-400 uppercase tracking-wider w-6 text-right">{item.unit}</span>
      </div>
    </div>
  )
}
